...
test("checking if handleDisplayDrawer is called", async () => {
    const mockHandleDisplayDrawer = jest.fn()

    render(<Notifications notifications={["notif1", "notif2"]} displayDrawer={false} handleDisplayDrawer={mockHandleDisplayDrawer} handleHideDrawer={() => {}}/>)

    await userEvent.click(screen.getByRole("paragraph"))
    expect(mockHandleDisplayDrawer.mock.calls).toHaveLength(1)
})


test("checking if handleHideDrawer is called", async () => {
    const mockHandleHideDrawer = jest.fn()

    render(<Notifications notifications={["notif1", "notif2"]} displayDrawer={true} handleDisplayDrawer={() => {}} handleHideDrawer={mockHandleHideDrawer}/>)

    await userEvent.click(screen.getByRole("close_button"))
    expect(mockHandleHideDrawer.mock.calls).toHaveLength(1)
